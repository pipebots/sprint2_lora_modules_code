...
CRC32_TABLE = micropython.const([0x00000000, 0x77073096, 0xEE0E612C,
...
def crc32_compute(data, crc32_checksum=0):
    crc32_checksum ^= 0xFFFFFFFF

    for item in data:
        crc32_checksum = (crc32_checksum >> 8) ^ \
                         CRC32_TABLE[(crc32_checksum & 0xFF) ^ item]
