"""CRC32 calculation for Pycom's implementation of micropython

Uses a pre-calculated table of values to compute a 32-bit, or 4-byte,
checksum of a sequence of bytes. Contains an auxiliary function for checking
which integer type, `short`, `int`, or `long` is 32-bit wide.

    Typical usage example:

    data = b'This is an example of a data input'
    checksum = crc32_compute(data)
"""
...
def crc32_compute(data: bytes, crc32_checksum: int = 0) -> bytes:
    ...
    https://rosettacode.org/wiki/CRC-32#Procedural, to calculate the 32-bit
    checksum of the input data.

    Args:
        data: The input data, in the form of `bytes`
    ...
    for item in data:
        crc32_checksum = (crc32_checksum >> 8) ^ CRC32_TABLE[
            (crc32_checksum & 0xFF) ^ item
        ]
